...
int parse_adj_matrix_file(const char* filename, vector_int *edges, int *n, int *m)
{
	...
	*n = 0;
	BeginPeekToken();
    while((tkType = GetNextToken()) != TK_NEWLINE) {
		if (tkType == TK_FLOATCONST)
			(*n)++;
	}
	EndPeekToken();
	...
	*m = 0;
	...
	for (int i = 0; i < *n; i++) {
		for (int j = 0; j < *n; j++) {
			if (tkType = GetNextToken() == TK_FLOATCONST && CurTokenStr[0] == '1')
				(*m)++;
		}
		tkType = GetNextToken();
	}
